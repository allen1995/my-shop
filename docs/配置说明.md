...
app:
  ...
  ai:
    # 主模型（默认使用dashscope）
    primary-model: ${AI_PRIMARY_MODEL:dashscope}
    # 备用模型（主模型失败时使用）
    fallback-model: ${AI_FALLBACK_MODEL:}
    # 模型配置
    models:
      dashscope:
        enabled: ${AI_DASHSCOPE_ENABLED:true}  # 是否启用DashScope模型
        priority: 1  # 优先级（数字越小优先级越高）
      openai:
        enabled: ${AI_OPENAI_ENABLED:false}  # 是否启用OpenAI模型（DALL·E）
        priority: 2
        api-key: ${OPENAI_API_KEY:}  # OpenAI API Key
      stability:
        enabled: ${AI_STABILITY_ENABLED:false}  # 是否启用Stable Diffusion模型
        priority: 3
        api-key: ${STABILITY_API_KEY:}  # Stability AI API Key
```

**配置说明：**
- `primary-model`: 指定主模型，默认为 `dashscope`
- `fallback-model`: 备用模型，当主模型失败时使用（可选）
- `models.dashscope.enabled`: 是否启用DashScope模型（默认启用）
- `models.openai.enabled`: 是否启用OpenAI模型（需要配置OPENAI_API_KEY）
- `models.stability.enabled`: 是否启用Stable Diffusion模型（需要配置STABILITY_API_KEY）

**环境变量配置示例：**
```bash
# 主模型选择
export AI_PRIMARY_MODEL=dashscope

# DashScope配置（已在上面配置）
export ALIYUN_DASHSCOPE_API_KEY=your-dashscope-api-key
...
# Stability AI配置（可选）
export AI_STABILITY_ENABLED=true
export STABILITY_API_KEY=your-stability-api-key
```

**多模型支持：**
- 当前已实现DashScope（阿里云百炼）模型
- OpenAI和Stable Diffusion模型支持将在后续版本中实现
- 系统会自动选择可用的模型，支持模型降级策略
